-- Apply before deploying the matching web routes. Payment attempt reservation,
-- provider idempotency and settlement must exist before the application uses them.
...
alter table public.orders
add column if not exists provider_idempotence_key text,
add column if not exists is_active_payment_attempt boolean not null default false,
add column if not exists reconciliation_reason text;

update public.orders
set provider_idempotence_key = gen_random_uuid()::text
where provider_idempotence_key is null;

alter table public.orders
alter column provider_idempotence_key set not null,
alter column provider_idempotence_key set default (gen_random_uuid()::text);
...
set is_active_payment_attempt = ranked_attempts.attempt_rank = 1
from ranked_attempts
where orders.id = ranked_attempts.id;
...
begin
  ...
  then
    raise exception using
      errcode = 'P0001',
      message = 'YooKassa job is not awaiting payment';
  end if;
  ...
  if not found then
    -- Adopt a pending attempt created by the pre-migration application during
    -- a rolling deploy. This keeps migration 0013 backward compatible.
    select *
    into payment_order
    from public.orders
    where job_id = p_job_id
...
begin
  ...
  if not found then
    raise exception using
      errcode = 'P0001',
      message = 'YooKassa payment attempt not found';
  end if;

  if payment_order.status <> 'pending'
    or not payment_order.is_active_payment_attempt
  then
    raise exception using
      errcode = 'P0001',
      message = 'YooKassa payment attempt is not active';
  end if;
...
create or replace function public.consume_user_photo_credit(
  p_user_id uuid
)
returns integer
language plpgsql
...
begin
  ...
  if not found then
    raise exception using
      errcode = 'P0001',
      message = 'Photo credit balance is empty';
  end if;
...
create or replace function public.refund_user_photo_credit(
  p_user_id uuid
)
returns integer
language plpgsql
...
begin
  ...
  if not found then
    raise exception using
      errcode = 'P0001',
      message = 'Photo credit profile not found';
  end if;
...
begin
  ...
  if not found or payment_job.payment_status <> 'paid' then
    raise exception using
      errcode = 'P0001',
      message = 'Generation job is not paid';
  end if;
...
revoke all on function public.reserve_yookassa_payment_attempt(
  uuid,
  uuid,
  text
) from public, anon, authenticated;

grant execute on function public.reserve_yookassa_payment_attempt(
  uuid,
  uuid,
  text
) to service_role;

revoke all on function public.finalize_yookassa_payment_attempt(
  uuid,
  uuid,
  uuid,
  text,
...
grant execute on function public.finalize_yookassa_payment_attempt(
  uuid,
  uuid,
  uuid,
  text,
...
revoke all on function public.settle_job_from_photo_balance(
  uuid,
  uuid
) from public, anon, authenticated;

grant execute on function public.settle_job_from_photo_balance(
  uuid,
  uuid
) to service_role;

revoke all on function public.delete_job_without_payment_history(
  uuid,
  uuid
) from public, anon, authenticated;

grant execute on function public.delete_job_without_payment_history(
  uuid,
  uuid
) to service_role;
...
revoke all on function public.consume_user_photo_credit(
  uuid
) from public, anon, authenticated;

grant execute on function public.consume_user_photo_credit(
  uuid
) to service_role;

revoke all on function public.refund_user_photo_credit(
  uuid
) from public, anon, authenticated;

grant execute on function public.refund_user_photo_credit(
  uuid
) to service_role;

revoke all on function public.record_generated_image_with_credit(
  uuid,
  uuid,
  uuid,
  text,
...
grant execute on function public.record_generated_image_with_credit(
  uuid,
  uuid,
  uuid,
  text,
...
revoke all on function public.grant_user_photo_credits(
  uuid,
  text,
  integer
) from public, anon, authenticated;

grant execute on function public.grant_user_photo_credits(
  uuid,
  text,
  integer
) to service_role;
